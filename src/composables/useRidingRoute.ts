/**
 * useRidingRoute —— 骑行路线规划 composable（Vue 3）。对应 react-bmap 的 useRidingRoute。
...
export function useRidingRoute(opts: RidingRouteOptions = {}): RidingRouteHookResult {
  ...
  const init = () => {
    ...
    if (handle.isNull) {
      ...
      error.value = new UnsupportedCapabilityError('RidingRoute', driver.version);
      return;
    }
    raw = handle.raw;
  ...
  return { data, loading, error, supported, search, clearResults, enableAutoViewport, disableAutoViewport, setPolicy, setLocation, setPolylineStyle, getStatus, cancel } as unknown as RidingRouteHookResult;
}
