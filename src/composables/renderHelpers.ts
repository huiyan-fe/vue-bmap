/**
 * 渲染型 service composable 的 autoRender 支持工具。
 *
 * 这些 composable（LocalSearch / 各路线 / BusLineSearch）只要拿到 Map 引用（无论经构造函数
 * location 还是 renderOptions.map），SDK 就会把结果自动渲染到地图上。autoRender:false 时
 * 需要两处都避开 Map：
 * - renderOptions.map 不注入（见各 composable 内 `if (dataOnly) delete ro.map`）
