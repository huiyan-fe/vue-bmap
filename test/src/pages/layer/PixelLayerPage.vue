...
import { Map, PixelLayer, useCapabilities, useDriver } from '@baidumap/vue-bmap';
import type { PixelLayerProps } from '@baidumap/vue-bmap';
import { BEIJING } from '../../TestProvider';

const caps = useCapabilities();
const driver = useDriver();
const supported = computed(() => caps.value.has('PixelLayer'));
const presence = computed<'present' | 'missing' | 'unknown'>(() => {
  const d = driver.value;
  if (!d) return 'unknown';
  const raw = d.rawSDK as Record<string, unknown> | null | undefined;
  if (!raw) return 'unknown';
  return typeof raw['PixelLayer'] === 'function' ? 'present' : 'missing';
});
...
const options = reactive<PixelLayerProps>({ visible: true });
...
const CODE = `<Map :default-center="center" :default-zoom="11">
  <PixelLayer visible :opacity="0.8" :z-index="3" />
</Map>`;
  ...
  <div class="test-page">
    <div class="test-map">
      <Map :default-center="BEIJING" :default-zoom="11" style="height:100%">
        <PixelLayer v-if="supported && mounted" v-bind="options" />
      </Map>
    </div>
    <div class="test-controls">
      ...
      <section>
        ...
        <p class="muted small">@since 4.0。像素图层，通过 addLayer / removeLayer 挂载。</p>
        <p class="muted small">
          左边是能力矩阵（按版本硬编码）的结论，右边是运行时在 SDK 上探测 <code>PixelLayer</code> 构造函数的结果。
